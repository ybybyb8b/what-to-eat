...
export function PlanDetailPage(){
  ...
  return <div className="sub-page plan-page"><PageHeader title="今晚的晚餐方案"/>
    ...
    <section className="detail-card"><h2>包含的菜品</h2>{recipes.map((recipe,index)=><article className="recipe-row" key={recipe.id}><span className={`dish-dot tone-${index+1}`}>{index+1}</span><div><strong>{recipe.name}</strong><p>{recipe.description}</p></div><span>{recipe.minutes} 分钟</span></article>)}</section>
