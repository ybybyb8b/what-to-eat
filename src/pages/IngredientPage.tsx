...
export function IngredientPage(){
  ...
  return <div className="sub-page ingredients-page"><PageHeader title="今天想买点什么" subtitle="选今晚想吃的，不用管家里库存"/>
    ...
    <div className="ingredient-grid">{visible.map((item)=>{const selected=selectedIngredientIds.includes(item.id);return <button key={item.id} className={selected?'ingredient-card selected':'ingredient-card'} onClick={()=>toggle(item.id)} aria-pressed={selected}>{item.image&&<img className="ingredient-image" src={item.image} alt="" aria-hidden="true"/>}<span className="ingredient-name">{item.name}</span>{selected&&<span className="check-badge"><Check/></span>}</button>})}</div>
