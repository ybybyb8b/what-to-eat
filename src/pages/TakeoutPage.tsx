import { Check, Edit3, Heart, Plus, Search, Tags, ThumbsDown, Trash2, UtensilsCrossed } from 'lucide-react'
...
function takeoutScore(item: TakeoutOption, favorite: boolean, rejected: number) {
  ...
  return (favorite ? 25 : 0) - rejected * 10 + (age < 3 ? -22 : age < 7 ? -10 : 0) - item.price * .03
}
...
export function TakeoutPage() {
  ...
  const saveCategories = (rows: CategoryRow[]) => {
    ...
    const fallback = names.includes('其他') ? '其他' : names[0]
    updateData((current) => ({
      ...current,
      takeouts: current.takeouts.map((item) => ({
        ...item,
        category: renameMap.get(item.category) ?? (takeoutCategories.includes(item.category) ? fallback : item.category)
      })),
  ...
  return <div className="sub-page takeout-page">
    ...
    <div className="takeout-list">{sorted.map(({ item, pref }, index) => <article className="takeout-card" key={item.id}>
      ...
      <div className="takeout-info">
        <div className="takeout-heading">
          <div>
            <span className="category-label">{item.category}</span>
            <h2>{item.name}</h2>
          </div>
          <strong>约 ¥{item.price}</strong>
        </div>
        <div className="takeout-footer">
          <small>{item.lastEatenAt ? `上次 ${new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric' }).format(new Date(item.lastEatenAt))}` : '最近没吃'}</small>
          <div className="takeout-actions">
            ...
            <button type="button" className="choose-takeout" onClick={() => {
...
function CategoryManager({ categories, takeouts, onClose, onSave }: {
  ...
  return <Modal title="管理外卖分类" onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      <p className="form-note">可以直接添加、重命名或删除。删除正在使用的分类后，其中候选会优先移到“其他”。</p>
      <div className="category-editor-list">{rows.map((row) => {
        ...
        return <div key={row.id}>
          ...
          <button type="button" className="delete-button" onClick={() => remove(row.id)} aria-label={`删除分类${row.name}`} title={count ? '删除并迁移其中候选' : '删除分类'}><Trash2 /></button>
        </div>
      })}</div>
